...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.ImdbRating > shows.Min(s => s.ImdbRating)).ToList();
        }
        ...
        {
            return shows.Where(s => s.ImdbRating < shows.Max(s => s.ImdbRating) && s.Genres.Contains("Drama")).ToList();
        }
        ...
        static Show FirstLongRunningTopRated(List<Show> shows)
        {
            return shows
                    .OrderBy(s => s.Name)
                    .FirstOrDefault(s => s.EndYear - s.StartYear > 10 && s.ImdbRating < 8);
        }

        // 21. Return the show with the most words in the name.
        static Show WordieastName(List<Show> shows)
        {
            return shows.FirstOrDefault(s => s.Name.Split(" ").Length == shows.Max(s => s.Name.Split(" ").Length));
        ...
        static string AllNamesWithCommasPlsAnd(List<Show> shows)
        {
            ...
            + " and "
            + shows.Select(s => s.Name).Skip(shows.Count - 1).FirstOrDefault();
        }
